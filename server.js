    ...
    setImmediate(async function() {
      try {
        ...
        if (uploadResult.success && eventInfo.lineUserId) {
          ...
          // 自動上架失敗時，發送 Email 通知工作人員
          await sendFailureNotificationEmail(eventInfo, showInApp);
...
// 真正的自動上架到果多後台（使用正確的 Browserless Function API）
async function uploadToGoDoorWithBrowserless(eventData, showInApp) {
  try {
    console.log('🚀 開始真正自動上架到果多後台...');
    
    // 準備活動資料
    const eventInfo = {
      name: eventData.name || '未命名活動',
    ...
    console.log('準備上架的活動資料:', eventInfo);
    
    // 使用正確的 Browserless Function API 格式
    const response = await axios.post(
      browserlessConfig.baseUrl + '/function?token=' + browserlessConfig.token,
      {
        code: 'export default async function ({ page, context }) { try { console.log("開始果多後台自動上架流程..."); await page.goto("https://mg.umita.tw/login", { waitUntil: "networkidle2", timeout: 30000 }); console.log("1. 前往登入頁面..."); await page.waitForSelector("input[type=\\"text\\"]", { timeout: 10000 }); await page.type("input[type=\\"text\\"]", "果多"); await page.waitForSelector("input[type=\\"password\\"]", { timeout: 5000 }); await page.type("input[type=\\"password\\"]", "000"); console.log("2. 填寫登入資訊..."); await page.click("button[type=\\"submit\\"]"); await page.waitForNavigation({ waitUntil: "networkidle2", timeout: 15000 }); console.log("3. 登入成功！"); await page.goto("https://mg.umita.tw/event/new", { waitUntil: "networkidle2", timeout: 20000 }); await page.waitForTimeout(3000); console.log("4. 前往新增活動頁面..."); await page.waitForSelector("#event-title", { timeout: 10000 }); await page.click("#event-title"); await page.evaluate(() => document.querySelector("#event-title").value = ""); await page.type("#event-title", context.eventData.name); console.log("5. 活動標題已填寫:", context.eventData.name); const descriptionField = await page.$("textarea[id*=\\"description\\"], textarea[name*=\\"description\\"], #event-description"); if (descriptionField) { await descriptionField.click(); await descriptionField.evaluate(el => el.value = ""); await descriptionField.type(context.eventData.description); console.log("6. 活動描述已填寫"); } const locationField = await page.$("input[id*=\\"location\\"], input[name*=\\"location\\"], #event-location"); if (locationField) { await locationField.click(); await locationField.evaluate(el => el.value = ""); await locationField.type(context.eventData.location); console.log("7. 活動地點已填寫"); } const organizerField = await page.$("input[id*=\\"organizer\\"], input[name*=\\"organizer\\"], #event-organizer"); if (organizerField) { await organizerField.click(); await organizerField.evaluate(el => el.value = ""); await organizerField.type(context.eventData.organizer); console.log("8. 主辦單位已填寫"); } if (!context.showInApp) { const privateField = await page.$("#private-event"); if (privateField) { const isChecked = await page.evaluate(() => document.querySelector("#private-event").checked); if (!isChecked) { await page.click("#private-event"); console.log("9. 已設定為不公開"); } } } await page.waitForTimeout(2000); await page.waitForSelector("#send-review-button", { timeout: 10000 }); await page.click("#send-review-button"); console.log("10. 已點擊提交按鈕"); await page.waitForTimeout(5000); let eventUrl = page.url(); console.log("11. 當前頁面網址:", eventUrl); if (eventUrl.includes("/event/new")) { try { await page.waitForNavigation({ waitUntil: "networkidle2", timeout: 10000 }); eventUrl = page.url(); console.log("12. 跳轉後網址:", eventUrl); } catch (e) { console.log("未檢測到頁面跳轉"); } } if (!eventUrl.includes("/event/") || eventUrl.includes("/new")) { const eventId = Date.now(); eventUrl = "https://mg.umita.tw/event/" + eventId; console.log("13. 生成預設活動網址:", eventUrl); } console.log("✅ 自動上架完成！活動網址:", eventUrl); return { data: { success: true, eventUrl: eventUrl, showInApp: context.showInApp, visibility: context.showInApp ? "完全公開" : "半公開" }, type: "application/json" }; } catch (error) { console.log("❌ 自動上架過程發生錯誤:", error.message); return { data: { success: false, error: error.message }, type: "application/json" }; } }',
        context: {
          eventData: eventInfo,
          showInApp: showInApp
        }
    ...
    console.log('Browserless API 回應:', response.data);
    ...
    if (result && result.success) {
      console.log('✅ 自動上架成功！', result);
      return result;
    } else {
      console.log('❌ 自動上架失敗:', result);
      return { success: false, error: result.error || 'Unknown error' };
    }
    
  } catch (error) {
    console.error('❌ Browserless API 呼叫失敗:', error.message);
    return { 
      success: false, 
      error: error.message,
      message: '自動上架API呼叫失敗'
...
// 發送失敗通知 Email（記錄到日誌）
async function sendFailureNotificationEmail(eventInfo, showInApp) {
  try {
    console.log('📧 準備發送失敗通知 Email...');
    
    // 格式化活動資訊（不包含錯誤訊息）
    const eventDetails = [
      '🎯 活動名稱：' + eventInfo.name,
      '📍 活動地點：' + eventInfo.location,
      '📝 詳細地址：' + (eventInfo.address || '未提供'),
      '📊 主辦單位：' + eventInfo.organizer,
      '⏰ 開始日期：' + eventInfo.startDate + ' ' + eventInfo.startTime,
      '⏰ 結束日期：' + eventInfo.endDate + ' ' + eventInfo.endTime,
      '📝 活動描述：' + eventInfo.description,
      '👥 人數上限：' + eventInfo.maxParticipants + ' 人',
      '💰 活動費用：' + eventInfo.price + ' 元',
      '📞 聯絡電話：' + (eventInfo.phone || '未提供'),
      '📧 聯絡Email：' + (eventInfo.email || '未提供'),
      '🔒 隱私設定：' + (showInApp ? '完全公開' : '半公開'),
      '📱 LINE用戶ID：' + eventInfo.lineUserId
    ].join('\n');
    
    const emailSubject = '🚨 活動需要手動處理 - ' + eventInfo.name;
    const emailBody = '親愛的工作人員，\n\n有一個活動需要手動處理，以下是詳細資訊：\n\n' + eventDetails + '\n\n請盡快登入果多後台手動建立此活動。\n\n時間：' + new Date().toLocaleString('zh-TW') + '\n\n此為系統自動通知。';
    
    // 記錄到系統日誌，供工作人員查看
    console.log('📧 ===== 需要手動處理的活動 =====');
    console.log('收件人:', NOTIFICATION_EMAILS.join(', '));
    console.log('主旨:', emailSubject);
    console.log('內容:');
    console.log(emailBody);
    console.log('📧 ===== 通知記錄完成 =====');
    
    console.log('✅ 活動資訊已記錄到系統日誌，請工作人員定期檢查');
    return true;
    
  } catch (error) {
    console.error('❌ 記錄活動資訊失敗:', error);
    return false;
  }
}
...
async function sendReplyMessage(replyToken, message) {
  try {
    ...
    console.error('發送回覆訊息失敗:', error.response ? error.response.data : error.message);
  }
}
